...
const HOUSEHOLD = ['저소득','장애인','다자녀','다문화','한부모·조손','다문화·탈북민','보훈대상자'];
...
const CategoryPage = () => {
  ...
  const doSearch = () => {
    ...
    if (selected.lifecycle.length) params.set('lifecycle', selected.lifecycle.join(','));
    if (selected.household.length) params.set('household', selected.household.join(','));
    if (selected.topics.length) params.set('topics', selected.topics.join(','));
  ...
  return (
    <div className="category-page">
      <div className="cat-top">
        ...
        <div className="cat-title">메인 화면 카테고리</div>
        <div style={{ width: 28 }} />
      </div>
      ...
      <div className="cat-footer">
        <button className="search-btn" onClick={doSearch}>검색하기</button>
      </div>
    </div>
  );
};
